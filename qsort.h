// this sort is hard to fit into the picture
// maybe need to play around with QCMP and a possible QCMP2 a bit...
...
{									\
  ...
  if (_elems > _QSORT_MAX_THRESH) {					\
    ...
    while (_QSORT_STACK_NOT_EMPTY) {					\
      ...
      QSORT_TYPE *_mid = _lo + ((_hi - _lo) >> 1);			\
									\
      if (QSORT_LT (_mid, _lo) QCMP)						\
        _QSORT_SWAP (_mid, _lo, _hold);					\
      if (QSORT_LT (_hi, _mid) QCMP)	{					\
      ...
      do {								\
        while (QSORT_LT (_left_ptr, _mid) QCMP)				\
         ++_left_ptr;							\
									\
        while (QSORT_LT (_mid, _right_ptr) QCMP)				\
          --_right_ptr;							\
									\
  ...
  {									\
    ...
    for (_run_ptr = _tmp_ptr + 1; _run_ptr <= _thresh; ++_run_ptr)	\
      if (QSORT_LT (_run_ptr, _tmp_ptr) QCMP)				\
        _tmp_ptr = _run_ptr;						\
									\
    if (_tmp_ptr != _base)						\
    ...
    while (++_run_ptr <= _end_ptr) {					\
      _tmp_ptr = _run_ptr - 1;						\
      while (QSORT_LT (_run_ptr, _tmp_ptr) QCMP)				\
        --_tmp_ptr;							\
									\
      ++_tmp_ptr;							\
